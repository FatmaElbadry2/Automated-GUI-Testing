...
dataset = 'CUSTOM'
dataset_root = 'data/CUSTOMdevkit/'
...
def train():
    global dataset, dataset_root, basenet, batch_size, resume, start_iter, \
        num_workers, cuda, lr, momentum, weight_decay, gamma, save_folder
    ...
    if dataset == 'CUSTOM':
        cfg = custom
        dataset = CUSTOMDetection(root=dataset_root, transform=SSDAugmentation(cfg['min_dim'], MEANS))
    ...
    for iteration in range(start_iter, cfg['max_iter']):
        ...
        if iteration != 0 and iteration % 5000 == 0:
            ...
            torch.save(ssd_net.state_dict(), 'weights/ssd300_CUSTOM_' +
                       repr(iteration) + '.pth')
    torch.save(ssd_net.state_dict(), 'weights/ssd300_CUSTOM_final.pth')
